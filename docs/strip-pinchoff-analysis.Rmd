...
This notebook contain the data analysis functions to analyze the strip pinchoff test (see LSPE-STRIP-PR-001 1.0)
...
```{python}
#import os
#os.chdir('/home/daniele/Documents/LSPE/testing_integration/system_level_tests/striptease')
import striptease
from importlib import reload
reload(striptease)
from striptease import hdf5files
import numpy as np
import pylab as pl
import pickle
...
```{python}
list(mcolors.CSS4_COLORS.values())[0]
```
...
```{python}
striptease.hdf5files.get_group_subgroup('ID1_SET')
```

```{python}
file_id = open('/home/daniele/Documents/LSPE/testing_integration/system_level_tests/striptease/output/2020_05_06_02-27-50_configuration.pickle','rb')
pinchoff_configuration = pickle.load(file_id)
file_id.close()
```
...
fname = '/home/daniele/Documents/LSPE/testing_integration/system_level_tests/pre_tests/strip-pinchoff/2020_05_06_02-27-50.h5' 

# Initialize the class
my_data = striptease.hdf5files.DataFile(fname)
...
```{python}
print(striptease.hdf5files.get_hk_descriptions('BIAS','POL'))
```

# Verification point 1


For each tile the first verification point is when all the modules have been turned on and there is a stable acquisition. So we look for the tag 'STABLE_ACQUISITION_X6' where X is the letter identifying the module
...
class PinchOffAnalysis:
    ...
    def __init__(self, data, output_folder = './'):
        ...
        self.verification_tags = self.get_subtags('PINCHOFF_VERIFICATION_1') + self.get_subtags('PINCHOFF_IDSET')
        self.configuration = {}
...
#        ['PINCHOFF_IDSET_%s_%s' % (p,a) for p in self.get_tested_polarimeters() for a in self.amps]
    ...
    def add_tag(self, \
        ...
        newtag = striptease.hdf5files.Tag(last_id, mjd_start, mjd_end, name, start_comment, end_comment)
    ...
    def get_times(self, polarimeter, tag):
        ...
        try:
            ...
            print(
                "Polarimeter %s is not present in data" % polarimeter
            )
            return None, None
    ...
    # Plot IV curves
    #########################################
    def plot_IV(self, polarimeters = 'All', filename = None):
        import striptease
        import pylab as pl
        import numpy as np
        ...
        for pol in polarimeters:
            ...
            highlight = [[[j for j in [1,2,3]] for i in [0,1]], [[j for j in [1,2,3]] for i in [0,1]]]
...
#                    print(pol, amps[row,col], amp_id, vg_value,id_value)
                    ...
                    for current in currents:
                        curtag = 'PINCHOFF_IDSET_%s_%s_%smuA' % (pol, amps[row,col], str(current))
                        tstart, tend = self.get_times(pol, curtag)

                        amp_id = self.amp_tag_translation[amps[row,col]]
                        cur_hk = 'VG%s_HK' % (str(amp_id))
                        group, subgroup = striptease.hdf5files.get_group_subgroup(cur_hk)
                        subgroup = '%s_%s' % (subgroup, pol)
                        t, vg_values = self.get_hk(group, subgroup, cur_hk, tstart, tend)

                        cur_hk = 'ID%s_HK' % (str(amp_id))
                        group, subgroup = striptease.hdf5files.get_group_subgroup(cur_hk)
                        subgroup = '%s_%s' % (subgroup, pol)
                        t, id_values = self.get_hk(group, subgroup, cur_hk, tstart, tend)
                    ...
                    try:
                        ...
                        rsquared2[row][col] = self.rsquare(dumv, dumi, fitquad[row][col], self.quad)
                    except:
                    ...
                    try:
                        ...
                        rsquared[row][col] = self.rsquare(dumv, dumi, fitlin[row][col], self.lin)
                    except:
                        fitlin[row][col] = None
                    ...
                    output[pol][amps[row,col]]['quadratic'] = (fitquad[row][col], rsquared2[row][col])
                    output[pol][amps[row,col]]['linear'] = (fitlin[row][col], rsquared[row][col])
                                    
            self.plot_IV_single(pol, vgate, idrain, fitquad, fitlin, rsquared2, rsquared, highlight = highlight)
...
#        return polarimeters
    ...
    def plot_IV_single(self, polarimeter, V, I, fitquad, fitlin, rq, rlin, highlight = False):
        import matplotlib
        matplotlib.use('Agg')
        ...
        for row in [0,1]:
            for col in [0,1,2]:
                ...
                axes[row,col].plot(V[row][col], I[row][col], 'o', markersize = 10, label = 'PINCHOFF TEST')
                xarr = np.linspace(min(V[row][col]), max(V[row][col]), 100)
                
                # Plot fits
                if fitquad[row][col] is not None:
                    equation = '$y = %3.4f\,x^2 + %3.4f\,x + %3.4f\,\,\, r^2 = %3.4f$' % \
                    (fitquad[row][col][0][0], fitquad[row][col][0][1], fitquad[row][col][0][2], rq[row][col])
                    axes[row,col].plot(xarr, self.quad(xarr, *fitquad[row][col][0]), label = equation)
                    
                if fitlin[row][col] is not None:
                    equation = '$y = %3.4f\,x + %3.4f\,\,\, r^2 = %3.4f$' % \
                    (fitlin[row][col][0][0], fitlin[row][col][0][1], rlin[row][col])
                    axes[row,col].plot(xarr, self.lin(xarr, *fitlin[row][col][0]), label = equation)
                
                # Labels
                axes[row,col].set(xlabel = 'Gate voltage [mV]', ylabel ='Drain current [$\mu$A]')
                ...
                if highlight != False:
                    ...
                    for index in indices:
                        ...
                        axes[row,col].plot(V[row][col][index], I[row][col][index], 'o', \
                                           markersize = 10, color = colors[20+j], label = lab)
                    
                title = 'Amplifier '+ amps[row, col]
                axes[row,col].set_title(title)
                axes[row,col].legend()
        
        filename = '%sIVplot_%s.pdf' % (self.output_folder, polarimeter)
        pl.savefig(filename)
        pl.close()
    ...
    def bias_plot(self, polarimeters = 'All', image = 'png'):
        ...
        '''
        vg_col = {'black': '#000000', 'blue': '#0000FF',\
        ...
        id_col = {'magenta': '#FF00FF', 'maroon': '#800000', \
                  ...
                  'skyblue': '#87CEEB', 'slateblue': '#6A5ACD', 'springgreen': '#00FF7F',\
            ...
            for row in [0,1]:
                for col in [0,1,2]:
                    ...
                    if cur_tag in self.configuration.keys():
                        ...
                        # VG data
                        group, subgroup = striptease.hdf5files.get_group_subgroup(cur_vghk)
                        subgroup = '%s_%s' % (subgroup, pol)
                        ...
                        #ID data
                        group, subgroup = striptease.hdf5files.get_group_subgroup(cur_idhk)
                        subgroup = '%s_%s' % (subgroup, pol)
...
#                    dumv = np.sort(dumv)
            ...
            self.bias_plot_single(pol, (timev, vgate), (timei, idrain), tags, vg_col, id_col, image = image)
...
#        return vgate, idrain, timev, timei    
    ...
    def bias_plot_single(self, polarimeter, vgate, idrain, tags, vg_col, id_col, image = 'png'):
        import matplotlib
        matplotlib.use('Agg')
        import matplotlib.pyplot as pl
        import matplotlib.colors as mcolors
        ...
        for row in [0,1]:
            for col in [0,1,2]:
                ...
                for x, y, x1, y1, t, vgc, idc in zip(newtime_v[row][col], vg[row][col], newtime_i[row][col], idr[row][col], tags, vg_col, id_col):
...
#                axes[row,col].set(xlabel = 'time [s]', ylabel = label)
        ...
        filename = '%s_vg_id_timeplot_%s.%s' % (self.output_folder, polarimeter, image)
        pl.savefig(filename)
        pl.close()
                    ...
                    if cur_tag in self.configuration.keys():
                        ...
                        for diode in diodes:
                            for data_type in data_types:
                                ...
                                dumv[key] = []
                                dumt[key] = []
                                t, values = self.get_sci(pol, data_type, diode, tstart, tend)
...
# #        file_id.close()
        ...
        for row in [0,1]:
            for col in [0,1,2]:
                ...
                for row1 in [0,1,2,3]:
                    for col1 in [0,1]:
                        key = keys1[row1][col1]
                        for x, y, t, in zip(newtime[key][row][col], data[key][row][col], tags):
    ...
    #                axes[row,col].set(xlabel = 'time [s]', ylabel = label)
                ...
                filename = '%s_sciplot_%s_%s.%s' % (self.output_folder, polarimeter, amps[row,col], image)
                pl.savefig(filename)
                pl.close()
    ...
    def save_configuration(self, d, filename, save = 'both'):
        ...
        if save == 'both' or save == 'csv':
            ...
            with open(self.output_folder + filename + '.csv',mode = 'w') as file_id:
                writer = csv.writer(file_id, delimiter = ',', quotechar='"', quoting=csv.QUOTE_MINIMAL)

                header_top = ['CHANNEL/STEP',\
                               'ID_HA3','ID_HA2','ID_HA1',\
                               'ID_HB3','ID_HB2','ID_HB1',\
                               'VG_HA3','VG_HA2','VG_HA1',\
                               'VG_HB3','VG_HB2','VG_HB1',\
                               'VG4A_HK','VG5A_HK']
                ...
                for pol in self.get_tested_polarimeters():
                    ...
                    try:
                        ...
                        print('Problem in configuration file. Either not present or wrong tag')
                    
                    head = ['PINCHOFF_VERIFICATION_1']
                    currents = self.get_currents(pol, 'HA1')
                    id_list = [data['ID%s_HK' % self.amp_tag_translation[h]] for h in ['HA3','HA2','HA1','HB3','HB2','HB1']]
                    vg_list = [data['VG%s_HK' % self.amp_tag_translation[h]] for h in ['HA3','HA2','HA1','HB3','HB2','HB1']]
...
#                        idvals = []
#                        vgvals = []
                        idvals = [self.configuration['PINCHOFF_IDSET_%s_%s_%smuA' % (pol, amp, cur)][pol]\
                                  ['ID%s_HK' % self.amp_tag_translation[amp]] for amp in ['HA3','HA2','HA1','HB3','HB2','HB1']]
                        vgvals = [self.configuration['PINCHOFF_IDSET_%s_%s_%smuA' % (pol, amp, cur)][pol]\
                                  ['VG%s_HK' % self.amp_tag_translation[amp]] for amp in ['HA3','HA2','HA1','HB3','HB2','HB1']]
...
#                            vgvals.append(self.configuration[tag][pol]['VG%s_HK' % self.amp_tag_translation[amp]])
    ...
    def get_subtags(self, search_tag, tags = None):
        ...
        substr = [search_tag]
        return [str for str in names if any(sub in str for sub in substr)] # Filter tags according to the search_tag
...
```{python}
pchoff = PinchOffAnalysis(my_data, output_folder='./output/')
```

```{python}
# Get start and end times for stable configuration and update tags
a = pchoff.get_times('R0','PINCHOFF_IDSET_R0_HB3_100muA')
start = a[0]
start_mjd = Time(start, format='mjd')
start_unix = start_mjd.unix
start_stable = Time(start_unix - 25*60., format='unix')
end_stable = Time(start_unix - 5*60., format='unix')
start_stable_mjd = start_stable.mjd
end_stable_mjd = end_stable.mjd

pchoff.add_tag(start_stable_mjd,end_stable.mjd,\
               'PINCHOFF_VERIFICATION_1',\
               'Start of stable acquisition before pinchoff','End of stable acquisition before pinchoff')
```

```{python}
pchoff.configuration = pinchoff_configuration
```

```{python}
pchoff.bias_plot(polarimeters = ['R0'])
```
...
```{python}
np.reshape(list(pluto.keys()),(4,2))
```

```{python}
pchoff.save_configuration(pinchoff_configuration,'2020_05_06_02-27-50_configuration', save = 'csv')
```

```{python}
pchoff.get_tested_polarimeters()
```

```{python}
pchoff.get_currents('B0','HB1')
```

```{python}
pchoff.get_subtags('PINCHOFF_IDSET_B0_HB1')
```

```{python}
pinchoff_configuration = pchoff.get_configuration(pchoff.get_tested_polarimeters())
```

```{python}
pchoff.save_configuration(pinchoff_configuration,'2020_05_06_02-27-50_configuration')
```
...
```{python}
fits = pchoff.plot_IV(polarimeters = ['B3'])
```

```{python}
pinchoff_configuration['PINCHOFF_VERIFICATION_1']['R0']
```

```{python}
pchoff.amp_tag_translation['HA3']
```
