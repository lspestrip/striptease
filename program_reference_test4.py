...
class ReferenceTestProcedure(StripProcedure):
    ...
    def run(self):
        ...
        for cur_board, pol_idx, polname in polarimeter_iterator(args.board):
            with StripTag(conn=self.command_emitter, name=f"ref4_turnon_pol_{polname}"):
            ...
            wait_with_tag(
                conn=self.conn, seconds=120, name="ref4_acquisition_default_pol{polname}"
            )
        ...
        for cur_board, pol_idx, polname in polarimeter_iterator(args.board):
            ...
            with StripTag(
                conn=self.command_emitter, name=f"ref4_set_pol_{polname}_phsw_{h}_STATE1"
            ):
        ...
        wait_with_tag(conn=self.conn, seconds=7200, name="ref4_wait_PHSW_STATE1")

        # STATE 2
        for cur_board, pol_idx, polname in polarimeter_iterator(args.board):

            with StripTag(
                conn=self.command_emitter, name=f"ref4_set_pol_{polname}_phsw_{h}_STATE2"
            ):
                for h, s in enumerate(
                    [
                        PhswPinMode.DEFAULT_STATE,
        ...
        wait_with_tag(conn=self.conn, seconds=7200, name="ref4_wait_PHSW_STATE2")

        # STATE 3
        for cur_board, pol_idx, polname in polarimeter_iterator(args.board):

            with StripTag(
                conn=self.command_emitter, name=f"ref4_set_pol_{polname}_phsw_{h}_STATE3"
            ):
        ...
        wait_with_tag(conn=self.conn, seconds=7200, name="ref4_wait_PHSW_STATE3")

        # STATE 4
        for cur_board, pol_idx, polname in polarimeter_iterator(args.board):

            with StripTag(
                conn=self.command_emitter, name=f"ref4_set_pol_{polname}_phsw_{h}_STATE4"
            ):
        ...
        wait_with_tag(conn=self.conn, seconds=7200, name="ref4_wait_PHSW_STATE4")
