...
class NoiseGenerator:
    ...
    def __init__(self):
        ...
        self.fmin = [
            1.0e-30,
            "%sfmin = %s%s: Minimum frequency to be considered\n"
        ]
        self.fmax = [
            1.0e30,
            "%sfmin = %s%s: Maximum frequency to be considered\n"
        ]
        self.A = [
            1.5e-5,
        ...
        self.Ns = [
            4,
            "%sNs = %s%s: number of amplifier stages.\n"
        ]
        self.corr = [
            True,
    ...
    def generate_noise(self,
        ...
        freq9        = np.double(frequency * 1.e9)
        beta9        = np.double(bandwidth * 1.e9)
        ...
        if self.corr[0]:
            seed_Tn = int(np.random.uniform()*1.e6)

        # Calculate the number of samples
        n_samples = time_length*samp_freq

        # Check if n_samples is even or odd
        check = np.double(n_samples)/2. - n_samples/2.
        ...
        if self.total_power[0]:
            ...
            C = 2.*np.sqrt(np.double(self.Ns[0]))*self.A[0]
            
            # Generate gain fluctuations
            sqrtA   = np.sqrt(2.)*(T_signal_ant + T_noise) * C * np.sqrt(samp_freq)
            noise_G = np.zeros(n_samples)
            if not self.wn_only[0]:
                noise_G = self.noise_kernel(1., sqrtA, samp_freq, slope, n_samples, seed_gain)

            # Generate noise temperature fluctuations        
            sqrtA    = np.sqrt(2.)*T_noise * self.A[0] * np.sqrt(samp_freq)
            noise_Tn = np.zeros(n_samples)
            if not self.wn_only[0]:
                noise_Tn = self.noise_kernel(1, sqrtA, samp_freq, slope, n_samples, seed_Tn)
        else:
            wn_rms   = parameters[0]
            ...
            noise_Tn = np.zeros(n_samples)
            if not self.wn_only[0]:
                noise_G = self.noise_kernel(parameters[1], sqrtA, samp_freq, slope, n_samples, seed_gain)
        # Add white noise
        np.random.seed(seed_wn)
        wn = np.random.normal(0., wn_rms, n_samples)
    ...
    def noise_kernel(self, fknee, sqrtA, samp_freq, slope, n_samples, seed):
        ...
        rms_wn_norm = 1./np.sqrt(2.*n_samples)
        ...
        sqr_spectrum = np.zeros(n_half) +1.

        indexarr     = np.array(range(n_half)) + 1
        funphi       = 2./np.pi*(np.arctan(np.double(indexarr)/new_f_min) -\
        ...
        np.random.seed(seed)
        noise           = np.random.normal(0., 1., n_half)
        ...
        creal[n_half]             = np.random.normal(0.,1.) * rms_wn_norm * sqr_spectrum[n_half-1]
        cimg[n_half]              = 0.

        result = (creal + 1j*cimg) * sqrtA
        #;    noise_G = 2d * np.double(n_samples) / sqrt(samp_freq) * np.double(fft(result,/inverse))
        noise = 2. * n_samples*np.double(np.fft.ifft(result))/np.sqrt(samp_freq)
