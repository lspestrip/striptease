...
def correlation_plot(
    ...
    ####################################################################################################################
        ...
        msg = (
            "Wrong data. Please insert only two of the four dataset: list1, list2, dict1 or dict2. "
    ...
    fig, axs = plt.subplots(
        nrows=n_rows, ncols=n_col, constrained_layout=True, figsize=fig_size
    )
    # Set the title
    ...
    if n_col == 1:
        ...
        label1 = (
            f"{data_name1} Temperature [K]"
            if data_name1[0] in ("T", "E")
            else f"{data_name1} {measure_unit1}"
        )
        label2 = (
            f"{data_name2} Temperature [K]"
            if data_name2[0] in ("T", "E")
            else f"{data_name2} {measure_unit2}"
        )
        ...
        # Interpolation
            ...
            x, y, label_x, label_y = fz.interpolation(
                list1, list2, time1, time2, label1, label2
            )
        # --------------------------------------------------------------------------------------------------------------
        ...
        if np.abs(correlation_value) > corr_t:
            warn_msg = (
                f"Found high correlation value: {round(correlation_value, 4)}"
            ...
            warnings["md"].append(
                f"|{data_name1}|{data_name2}|{round(correlation_value, 4)}|\n"
            )
            ...
            warnings["csv"].append([""])
            warnings["csv"].append(
                [f"{data_name1}", f"{data_name2}", f"{round(correlation_value, 4)}"]
            )
    # ------------------------------------------------------------------------------------------------------------------
            ...
            for r, r_exit in enumerate(dict1.keys()):
                for c, c_exit in enumerate(dict2.keys()):
                    ...
                    ####################################################################################################
                        ...
                        # Down-sampling
                        if down_sampling:
                            x, y, label_x, label_y = fz.down_sampling(
                                sub_set1, sub_set2, label1, label2
                            )

                        # Interpolation
                        else:
                            x, y, label_x, label_y = fz.interpolation(
                                sub_set1, sub_set2, time1, time2, label1, label2
                            )
                        # ----------------------------------------------------------------------------------------------
            ...
            if self_correlation:
                ...
                for i in range(len(keys) - 1):
                    if np.abs(correlation_matrix.loc[keys[i], keys[i + 1]]) > corr_t:
                        warn_msg = (
                            f"Found high correlation value between {keys[i]} and {keys[i + 1]}: "
                ...
                for key1 in df1.columns:
                    for key2 in df2.columns:
                        ...
                        if np.abs(correlation_matrix.loc[key1, key2]) > corr_t:
                            warn_msg = (
                                f"Found high correlation value between {key1} and {key2}: "
        ...
        ################################################################################################################
            ...
            for c, exit in enumerate(dict1.keys()):
                ...
                if down_sampling:
                    x, y, label_x, label_y = fz.down_sampling(
                        list1, sub_set1, label1, label2
                    )

                # Interpolation
                else:
                    x, y, label_x, label_y = fz.interpolation(
                        list1, sub_set1, time1, time2, label1, label2
                    )
                # ------------------------------------------------------------------------------------------------------
                ...
                if np.abs(correlation_value) > corr_t:
                    warn_msg = (
                        f"Found high correlation value between {data_name1} and {data_name2} "
                    ...
                    # [MD] Collecting the warning
                    warnings["md"].append(
                        f"|{data_name1}|{data_name2} {exit}|{round(correlation_value, 4)}|\n"
                    )
...
def correlation_mat(
    ...
    fig.suptitle(
        f"Correlation Matrix {data_name} - Date: {start_datetime}", fontsize=12
    )

    pl_m1 = sn.heatmap(correlation_matrix, annot=True, ax=axs[0], cmap="coolwarm")
    pl_m1.set_title(f"Correlation {data_name}", fontsize=14)
    pl_m2 = sn.heatmap(
        correlation_matrix, annot=True, ax=axs[1], cmap="coolwarm", vmin=-0.4, vmax=0.4
    )
                ...
                for n_1 in range(len(polar_names)):
                    for n_2 in range(n_1 + 1, len(polar_names)):
                        ...
                        if np.abs(correlation_matrix[name_1][name_2]) > corr_t:
                            warn_msg = (
                                f"Found high correlation value between "
                            ...
                            warnings["csv"].append(
                                [
                                    f"{name_1} {exit_1} {kind}",
                                    f"{name_2} {exit_2} {kind}",
                                    f"{correlation_matrix[name_1][name_2]}",
                                ]
                            )
