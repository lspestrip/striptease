...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="PROGRAM",
        description="Official pipeline for the functional verification "
    ...
    common_parser.add_argument(
        "--hk_sam_exp_med",
        "-hk_sem",
        type=lambda x: [float(val) for val in x.split(",")],
        default=[1.4, 1.4, 60.0, 60.0, 60.0],
    ...
    subparsers = parser.add_subparsers(
        help="Operation modalities of the pipeline.", dest="subcommand"
    )
    ...
    parser_A.add_argument(
        "name_pol",
        type=str,
        help="- str containing the name(s) of the polarimeter(s). "
    ...
    if toml_usage:
        if not Path(args.toml_file_path).exists():
            logging.error(
                f"The target directory {args.toml_file_path} does not exist. "
    ...
    try:
        ...
        logging.error(
            f"The parameter {args.path_file} should be a string. Please check it again.\n"
            f"Type -h for help!\n\n"
        )
        raise SystemExit(1)
    ...
    if not Path(args.path_file).exists():
        logging.error(
            f"The target directory {args.path_file} does not exist or does not contain the hdf5 file index.\n"
    ...
    if args.end_datetime < args.start_datetime:
        logging.error(
            "end_datetime is before than start_datetime: wrong datetime values.\n\n"
        )
        raise SystemExit(1)
    ...
    if args.end_datetime == args.start_datetime:
        logging.error(
            "end_datetime is equal to start_datetime: wrong datetime values.\n\n"
        )
        raise SystemExit(1)
    ...
    if (Time(args.end_datetime) - Time(args.start_datetime)).sec < 120:
        logging.error(
            "The duration of the analysis is too short: please make sure that the difference between "
    ...
    if args.subcommand == "tot" or args.subcommand == "pol_hk":
        ...
        if not fz.name_check(name_pol):
            logging.error(
                "The names of the polarimeters provided are not valid. Please check the parameter name_pol. "
    ...
    if args.subcommand == "tot":
        ...
        if not isinstance(args.thermal_sensors, bool):
            logging.error(
                "The parameter thermal_sensors must be a bool. Please check it again.\n\n"
            )
            raise SystemExit(1)
        # --------------------------------------------------------------------------------------------------------------
        ...
        if not isinstance(args.housekeeping, bool):
            logging.error(
                "The parameter housekeeping must be a bool. Please check it again.\n\n"
            )
            raise SystemExit(1)
        # --------------------------------------------------------------------------------------------------------------
        ...
        if not isinstance(args.scientific, bool):
            logging.error(
                "The parameter scientific must be a bool. Please check it again.\n\n"
            )
            raise SystemExit(1)
        # --------------------------------------------------------------------------------------------------------------
        ...
        if not args.even_odd_all.issubset({"E", "O", "A"}):
            logging.error(
                "Wrong Data Name: please choose between the options: E, O, A, EO, EA, OA, EOA.\n\n"
            )
            raise SystemExit(1)
        # --------------------------------------------------------------------------------------------------------------
        ...
        if not isinstance(args.rms, bool):
            logging.error(
                "The parameter rms must be a bool. Please check it again.\n\n"
            )
            raise SystemExit(1)
        # --------------------------------------------------------------------------------------------------------------
        ...
        if not isinstance(args.smooth, int):
            logging.error(
                "The parameter smooth must be an int. Please check it again.\n\n"
            )
            raise SystemExit(1)
        # --------------------------------------------------------------------------------------------------------------
        ...
        if not isinstance(args.window, int):
            logging.error(
                "The parameter window must be an int. Please check it again.\n\n"
            )
            raise SystemExit(1)
        # --------------------------------------------------------------------------------------------------------------
    ####################################################################################################################
    ...
    if args.subcommand == "tot" or args.subcommand == "thermal_hk":
        if not isinstance(args.fourier, bool):
            logging.error(
                "The parameter fourier must be a bool. Please check it again.\n\n"
            )
            raise SystemExit(1)
    # ------------------------------------------------------------------------------------------------------------------
    ...
    if args.subcommand == "tot":
        if not isinstance(args.nperseg, int):
            logging.error(
                "The parameter nperseg must be an int. Please check it again.\n\n"
            )
            raise SystemExit(1)
    # ------------------------------------------------------------------------------------------------------------------
    ...
    if args.subcommand == "tot" or args.subcommand == "thermal_hk":
        if not isinstance(args.nperseg_thermal, int):
            logging.error(
                "The parameter nperseg_thermal must be an int. Please check it again.\n\n"
            )
            raise SystemExit(1)
    # ------------------------------------------------------------------------------------------------------------------
    ...
    if args.subcommand == "tot" or args.subcommand == "thermal_hk":
        ...
        if not isinstance(args.spike_data, bool):
            logging.error(
                "The parameter spike_data must be a bool. Please check it again.\n\n"
            )
            raise SystemExit(1)
        # --------------------------------------------------------------------------------------------------------------
        ...
        if not isinstance(args.spike_fft, bool):
            logging.error(
                "The parameter spike_fft must be a bool. Please check it again.\n\n"
            )
            raise SystemExit(1)
        # ------------------------------------------------------------------------------------------------------------------
    ...
    if args.subcommand == "tot":
        try:
            ...
            logging.error(
                "The parameter sam_tolerance must be a float. Please check it again.\n\n"
            )
            raise SystemExit(1)
    # ------------------------------------------------------------------------------------------------------------------
    ...
    if args.subcommand == "tot" or args.subcommand == "pol_hk":
        for i in range(len(args.hk_sam_exp_med)):
            try:
                ...
                logging.error(
                    "The 3 parameters of hk_sam_exp_med must be float. Please check them again.\n\n"
                )
                raise SystemExit(1)
    # ------------------------------------------------------------------------------------------------------------------
    ...
    if args.subcommand == "tot" or args.subcommand == "pol_hk":
        for i in range(len(args.hk_sam_tolerance)):
            try:
                ...
                logging.error(
                    "The 3 parameters of hk_sam_tolerance must be float. Please check them again.\n\n"
                )
                raise SystemExit(1)
    # ------------------------------------------------------------------------------------------------------------------
    ...
    hk_sam_exp_med = {
        "I": args.hk_sam_exp_med[0],
        "V": args.hk_sam_exp_med[1],
        "O": args.hk_sam_exp_med[2],
        "M": args.hk_sam_exp_med[3],
        "P": args.hk_sam_exp_med[4],
    }
    hk_sam_tolerance = {
        "I": args.hk_sam_tolerance[0],
        "V": args.hk_sam_tolerance[1],
        "O": args.hk_sam_tolerance[2],
        "M": args.hk_sam_tolerance[3],
    ...
    if args.subcommand == "tot" or args.subcommand == "thermal_hk":
        try:
            ...
            logging.error(
                "The parameter ts_sam_exp_med must be a float. Please check it again.\n\n"
            )
            raise SystemExit(1)
    # ------------------------------------------------------------------------------------------------------------------
    ...
    if args.subcommand == "tot" or args.subcommand == "thermal_hk":
        try:
            ...
            logging.error(
                "The parameter ts_sam_tolerance must be a float. Please check it again.\n\n"
            )
            raise SystemExit(1)
    # ------------------------------------------------------------------------------------------------------------------
    ...
    if not isinstance(args.corr_plot, bool):
        logging.error(
            "The parameter corr_plot must be a bool. Please check it again.\n\n"
        )
        raise SystemExit(1)
    # ------------------------------------------------------------------------------------------------------------------
    ...
    if not isinstance(args.corr_mat, bool):
        logging.error(
            "The parameter corr_mat must be a bool. Please check it again.\n\n"
        )
        raise SystemExit(1)
    # ------------------------------------------------------------------------------------------------------------------
    ...
    try:
        ...
        logging.error(
            "The parameter corr_t must be a float. Please check it again.\n\n"
        )
        raise SystemExit(1)
    # ------------------------------------------------------------------------------------------------------------------
    ...
    if args.subcommand == "tot":
        if not isinstance(args.cross_corr, bool):
            logging.error(
                "The parameter cross_corr must be a bool. Please check it again.\n\n"
            )
            raise SystemExit(1)
    # ------------------------------------------------------------------------------------------------------------------
    ...
    if not isinstance(args.output_plot_dir, str):
        logging.error(
            "The parameter output_plot_dir must be a str. Please check it again.\n\n"
        )
        raise SystemExit(1)
    # ------------------------------------------------------------------------------------------------------------------
    ...
    if not isinstance(args.output_report_dir, str):
        logging.error(
            "The parameter output_report_dir must be a str. Please check it again.\n\n"
        )
        raise SystemExit(1)
    # ------------------------------------------------------------------------------------------------------------------
    ...
    if not isinstance(args.report_to_plot, str):
        logging.error(
            "The parameter report_to_plot must be a str. Please check it again.\n\n"
        )
        raise SystemExit(1)
    # ------------------------------------------------------------------------------------------------------------------
    ...
    if args.subcommand == "tot":
        ...
        strip_a.tot(
            ...
            thermal_sensors=args.thermal_sensors,
            housekeeping=args.housekeeping,
            sam_tolerance=args.sam_tolerance,
            hk_sam_exp_med=hk_sam_exp_med,
            hk_sam_tolerance=hk_sam_tolerance,
            ts_sam_exp_med=args.ts_sam_exp_med,
            ts_sam_tolerance=args.ts_sam_tolerance,
            name_pol=args.name_pol,
            eoa=args.even_odd_all,
            ...
            nperseg_thermal=args.nperseg_thermal,
            spike_data=args.spike_data,
            spike_fft=args.spike_fft,
            corr_plot=args.corr_plot,
            corr_mat=args.corr_mat,
            corr_t=args.corr_t,
    ...
    if args.subcommand == "tot":
        ...
        report_name = fz.dir_format(
            f"00_Report_{args.start_datetime}__{args.end_datetime}"
        )
        # Merge all the MD report into a single file
        logging.info(f"Merging all MD reports into: {report_name}.md\nEnjoy!\n\n")
        fz.merge_report(
            md_reports_path=args.output_report_dir,
        ...
        # JSON PRODUCTION
        ################################################################################################################
        logging.info("Converting CSV report into JSON\n")
        # Convert the CSV Report File into a JSON Report File
        fz.csv_to_json(
            csv_file_path=f"{args.output_report_dir}/CSV/{report_name}.csv",
