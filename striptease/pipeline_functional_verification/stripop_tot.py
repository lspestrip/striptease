...
def tot(
    ...
    spike_fft: bool,
    sam_tolerance: float,
    hk_sam_exp_med: [float, float, float],
    hk_sam_tolerance: [float, float, float],
    ts_sam_exp_med: float,
    ts_sam_tolerance: float,
    corr_plot: bool,
    corr_mat: bool,
    corr_t: float,
    cross_corr: bool,
    ...
    with open(f"{csv_output_dir}/{report_name}.csv", "a", newline="") as file:
    ...
    with open(f"{output_report_dir}/{noise_report_name}", "w") as file:
        file.write(
            "Polarimeter_Name\tData_Type\tChannel\tWN_level\t1_f_slope\tknee_freq\n"
        )
    ...
    if not thermal_sensors:
        ...
        for status in [0, 1]:
            TS = ts.Thermal_Sensors(
                path_file=path_file,
                start_datetime=start_datetime,
                end_datetime=end_datetime,
                status=status,
                nperseg_thermal=nperseg_thermal,
            ...
            sampling_table = TS.TS_Sampling_Table(
                sam_exp_med=ts_sam_exp_med, sam_tolerance=ts_sam_tolerance
            )
            ...
            if spike_data:
                ...
                with open(
                    f"{csv_output_dir}/{report_name}.csv", "a", newline=""
                ) as file:
                    writer = csv.writer(file)
                    writer.writerows(csv_general)
                logging.info(
            ...
            if spike_fft:
                ...
                with open(
                    f"{csv_output_dir}/TS_Report_{start_datetime}__{end_datetime}.csv",
                    "a",
                    newline="",
                ) as file:
                    writer = csv.writer(file)
            ...
            logging.info(
                f"Plotting all TS measures for status {status} of the multiplexer."
            )
            TS.Plot_TS()
            ...
            if fft:
                logging.info(
                    f"Plotting the FFT of all the TS measures for status {status} of the multiplexer."
                )
    ...
    if cross_corr:
        ...
        cross_corr_mat = fz_c.cross_corr_mat(
            path_file=path_file,
            start_datetime=start_datetime,
            end_datetime=end_datetime,
            show=False,
    ...
    for np in name_pol:
        ...
        if not housekeeping:
            ...
            HK_sampling_table = p.HK_Sampling_Table(
                sam_exp_med=hk_sam_exp_med, sam_tolerance=hk_sam_tolerance
            )
            # [MD]
        ...
        for type in ["DEM", "PWR"]:
                ...
                for combo in combos:
                    ...
                    if all(value == 0 for value in combo):
                        ...
                        if rms:
                            ...
                            logging.info(
                                f"Plotting Even Odd All Outputs RMS. Type {type}."
                            )
                            fz.data_plot(
                                pol_name=np,
                                dataset=p.data,
                        ...
                        if fft:
                            ...
                            if rms:
                                ...
                                logging.info(
                                    f"Plotting Even Odd All FFT of the RMS. Type {type}."
                                )
                                fz.data_plot(
                                    pol_name=np,
                                    dataset=p.data,
            ...
            if not scientific:
                ...
                if fft:
                    ...
                    if rms:
                        logging.info(
                            f"Plot of FFT of the RMS of Scientific Data. Type {type}."
                        )
                        fz.data_plot(
                            pol_name=np,
                            dataset=p.data,
            ...
            if corr_plot or corr_mat:
                ...
                if not scientific:
                    ...
                    for type in ["DEM", "PWR"]:
                        ...
                        for exit in ["Q1", "Q2", "U1", "U2"]:
                            sci_data[exit] = fz.demodulate_array(
                                p.data[type][exit], type
                            )
                ...
                for d1, t1, n1, u1, d2, t2, n2, u2 in possible_combos:
                    ...
                    if corr_plot:
                        ...
                        correlation_warnings = fz_c.correlation_plot(
                            list1=[],
                            list2=[],
                            dict1=d1,
                            dict2=d2,
                    ...
                    if corr_mat:
                        ...
                        correlation_warnings = fz_c.correlation_mat(
                            dict1=d1,
                            dict2=d2,
                            data_name1=f"{n1}",
                            data_name2=f"{n2}",
        ...
        if not thermal_sensors:
            ...
            possible_combos.extend(
                [
                    # TS 0 vs TS 1
                    (
                        ts_0.ts["thermal_data"]["calibrated"],
                        ts_0.ts["thermal_times"],
                        "TS_status_0",
                        "Temperature [K]",
                        ts_1.ts["thermal_data"]["calibrated"],
                        ts_1.ts["thermal_times"],
                        "TS_status_1",
                        "Temperature [K]",
                    ),
                    # TS 0 self correlation
    ...
    if corr_plot:
        ...
        png_files = sorted(
            [
                file
                for file in os.listdir(f"{output_plot_dir}/Correlation_Plot/")
    ...
    if corr_mat:
        ...
        png_files = sorted(
            [
                file
                for file in os.listdir(f"{output_plot_dir}/Correlation_Matrix/")
                if file.lower().endswith(".png")
            ]
        )
        ...
        if cross_corr:
            ...
            png_cross_files = sorted(
                [
                    file
                    for file in os.listdir(f"{output_plot_dir}/Cross_Corr/")
                    if file.lower().endswith(".png")
                ]
            )
