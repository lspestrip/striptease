...
# Util functions
def get_string_from_tag(tags_arr,idtags,
	relational_op='start',idx_str=1):
	"""
...
def get_info_tag_start(tags_arr,idtags):
	...
	for ii,tt in enumerate(tags_arr['tag'].astype(str)):
		if tt.startswith(idtags):
			...
			out.append([tags_arr['id'][ii],tags_arr['mjd_start'][ii],
				tags_arr['mjd_end'][ii],tags_arr['start_comment'][ii],
...
def get_selection_param(datfile,param,t0,t1=None):
	...
	if ncount==0:
		...
		tpar,vpar = tt[-1],vv[-1]
	#
	dictOut = {'count':ncount,'param':param,
	'time_par':tt,'value_par':vv}
...
def load_one_curve_polna(datfile,tags_arr,pol,lna,tag_ref='SET_VGVD_'):
    ...
    tagcur_all = get_string_from_tag(
    	tags_arr,idcur_in,relational_op='in'
    	)
    #
    ...
    curAll = np.zeros([np.max(vg_idx)+1,np.max(vd_idx)+1],
    	[('DrainI', '<f4'),('GateV', '<f4'),('DrainV', '<f4')])
    curInfo = np.zeros([np.max(vg_idx)+1,np.max(vd_idx)+1],
...
class IVcurve_analysis:
	...
	def query_ivcurves(self,silent=True):
		...
		for pp in pol_all:
			...
			dictLnas={}
			for ll in lna_in:
				cur0,count0 = load_one_curve_polna(self.dfile,tags,pp,ll,tag_ref='SET_VGVD_')
	...
	def plotting_ivcurve(self,idout=None):
		...
		for pp in pol_module:
			...
			for ll in lna_name:
				...
				plt.subplot(1,2,1)
				plt.title(f"{pp} ({ll})")
				plt.xlabel('VD [mV]'); plt.ylabel('ID [uA]')
