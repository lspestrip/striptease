...
def find_waittime_tests(ds:DataStorage,date_range:Union[Tuple[float, float],Tuple[str,str],Tag,Tuple[Time,Time]],
                        tag_main:str="WAITTIME",tag_exclude:Union[List[str],str]='VERIFICATION_TURNON')->List[Tag]:
    """
    ...
    idcase = str(date_str[0].fits).split('.')[0]
    polna = str(tt.name).split(tag_main+'_')[1]
    print(f"Analysing Case {idx_case}: {idcase} ({polna})")
    #f"from {date_str[0].fits.replace('T', ' ')} to {date_str[1].fits.replace('T', ' ')}")
    print(f"\t{tt}")
    #
    return tags_out

def load_waittime_polan_analysis(ds_in:DataStorage,tag_in:Tag,tag_main:str="WAITTIME")->Dict:
    """
    ...
    results = {'idcase':idcase,'polna':polna,'pol_name':pol_name,'lna_name':lna_name,
               'time_start':t0}
    ...
    arr_wtime,arr_case,arr_sets = np.unique(tids[:,0]),np.unique(tids[:,3]),np.unique(tids[:,2])
    nwt,ncase,nsets = len(arr_wtime), len(arr_case), len(arr_sets)
    ...
    for tt in tags:
        if '_SET_VD_' in tt.name:
            ...
            arr_volt[cc] = vvolt
    log.info(f"Waittime [sec] ({nwt}): {arr_wtime}\nVoltages [mV]  ({ncase}): {arr_volt}")
    #print(f"Voltages [mV]  ({ncase}): {arr_volt}")
    ...
    results.update({f'{arr_sets[0]}_date_range':np.zeros([nwt,ncase],dtype_dranges),
                    f'{arr_sets[1]}_date_range':np.zeros([nwt,ncase],dtype_dranges),
                    f'{arr_sets[0]+arr_sets[1]}_date_range':np.zeros([nwt,ncase],dtype_dranges),
                    f'GAP_date_range':np.zeros([nwt,ncase],dtype_dranges),})
    idx_wt = {}
    for ii,ww in enumerate(arr_wtime): idx_wt[str(ww)]=ii
    ...
    results[f'{set_joint}_date_range']['t_start'] = results[f'{arr_sets[0]}_date_range']['t_start']
    results[f'{set_joint}_date_range']['t_end'] = results[f'{arr_sets[1]}_date_range']['t_end']
    set_gap = 'GAP'
    results[f'{set_gap}_date_range']['t_start'] = results[f'{arr_sets[0]}_date_range']['t_end']
    results[f'{set_gap}_date_range']['t_end'] = results[f'{arr_sets[1]}_date_range']['t_start']
    ...
    dtype_results = [('count', '<f4'), ('DrainV', '<f4'), ('t_DrainV', '<f4'),
                     ('WaitTime', '<f4'),('t_start', '<f4'), ('t_end', '<f4'),
                     ('WaitTime_in', '<f4'), ('DrainV_in', '<f4')]

    for sm in set_measure:
        res0 = np.zeros([nwt,ncase],dtype_results)
        #print(f"\n####### Data set: {sm}")
        for ii,wt in enumerate(arr_wtime):
            log.info(f"Configuration: set {sm} for waitTime {wt}sec ({ii+1}/{nwt}). Analyzing {ncase} cases of voltages:")
            for jj,vv in track(enumerate(arr_volt),description=f"Progress..."):
                ...
                cc,vd,t_vd = get_polna_statistic(ds_in,vrange,pol_name,lna_name,parin="VD")
                res0[ii,jj] = (cc,vd,t_vd,deltat,vrange[0],vrange[1],wt,vv)
                #print(f'  Yield: {res0[ii,jj]}\n')
        results[sm] = res0
    return results

def get_polna_statistic(ds:DataStorage,date_range:Union[Tuple[float, float],Tuple[str,str],Tag,Tuple[Time,Time]],
                  pol_name:str,lna_name:str,parin:str='VD',statistic='last') -> Tuple:
    """
    ...
    #print(f"\tLoading HK ({type(date_range)}, {date_range}, '{subgroup}', '{lna_name}', '{par_str}')") #This took around 9 sec by one case
    vtime, vparam = ds.load_hk(mjd_range=date_range, group="BIAS",
                             subgroup=subgroup, par=par_str)
    #print(f'\t--> Time: {vtime} ## Params: {vparam}')
...
def plotting_countmap(metaData,idout=None,path_dir:Path= Path('./')):
    ...
    polna, pp, ll = md['polna'], md['pol_name'], md['lna_name']
    vsets = md['sets']
    #
    #print(f"\n## Plotting the number count in each configuration: {polna}")
    ...
    for jj, kk in enumerate(vsets[0:3]):
        ...
        plt.imshow(curAll['count'], vmin=-0.5, vmax=8.5, cmap="Set1",
                   extent=vextent, aspect='auto', origin='lower')
        if jj == 2:
...
def plotting_checks_params(metaData, idout=None,plot_case:Union[str,Dict]='WT',path_dir:Path= Path('./')):
    """
    ...
    vsets = md['sets']
    #
    #print(f"\n## Plots to check input and measured params: {polna}")

    ## Plot case
    if plot_case == 'WT':
        pcase = {'set':'WT','column':'WaitTime','label':'sec','idoutput':'waittime'}
    elif plot_case == 'VD':
    ...
    kk0 = pcase['set']
    curAll = {'input':md[kk0][pcase["column"]+'_in'],'get':md[kk0][pcase["column"]]}
    ...
    for kk, cur in curAll.items():
        ...
        plt.imshow(cur, vmin=vmin, vmax=vmax, cmap="jet",
                   extent=vextent, aspect='auto', origin='lower')

        if kk != 'input':
            cbar = plt.colorbar(pad=0.02,fraction=0.15)
            cbar.ax.tick_params(labelsize=8)
            cbar.set_label(pcase['label'], labelpad=-20, y=-0.02, rotation=0,fontsize=10)
        jj += 1
...
def plotting_settingTime(metaData, idout=None,path_dir:Path= Path('./')):
    ...
    vsets = md['sets']
    #
    #print(f"\n## Plotting the setting time of each data set: {polna}")
    ...
    for jj, kk in enumerate(vsets):
        ...
        plt.imshow(curAll['WaitTime'], cmap="jet",
                   extent=vextent, aspect='auto', origin='lower')
...
class checks_waitTime_analysis:
    ...
    def __init__(self,args):
        ...
        if args.picklein:
            ...
            log.info(f"Getting the metaData")
    ...
    def querry_metaData(self)->Dict:
        ...
        metaData = load_waittime_polan_analysis(ds_in=self.ds, tag_in=tag_ini[0], tag_main="WAITTIME")
    ...
    def mkdir_report(self,reportTemplate_file:str=None):
        ...
        if reportTemplate_file is None:
            path_template = Path(st.__path__[0]).parent / "templates"
    ...
    def run(self):
        ...
        # Ploting
        log.info(f"Plotting maps")
        log.info(f"Plotting the count map")
        plotting_countmap(self.metaData,idout=self.idcase,path_dir=self.output_dir)
        log.info(f"Plotting the check map of the wait time inputs")
        plotting_checks_params(self.metaData,idout=self.idcase,plot_case='WT',path_dir=self.output_dir)
        log.info(f"Plotting the check map of the voltage inputs")
        plotting_checks_params(self.metaData,idout=self.idcase,plot_case='VD',path_dir=self.output_dir)
        log.info(f"Plotting the check map of the setting times for each data set")
        ...
        self.figname = {
            ...
            'waittime':self.output_dir.absolute()/f"Fig_{self.idcase}_waitTimeMap.png",
            'voltage':self.output_dir.absolute()/f"Fig_{self.idcase}_voltageMap.png",
            'settingtime':self.output_dir.absolute()/f"Fig_{self.idcase}_settingTimeMap.png"
        }
        ...
        log.info(f"Time run (main routine): {time_run_sec} sec ({time_run_sec / 60:.3f} min)")
        # END main program
