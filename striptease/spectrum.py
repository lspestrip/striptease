...
class Spectrum:
    ...
    def __init__(self):
        ...
        *KEYWORDS*
        ...
        self.lowfreq       = [1.e-3, '%slowfreq = %s%s: Minimum frequency to be considered in Welch '
                                     'windowing and segmenting. It is  used only if ' 
                                     'the key welch = True\n']
        self.welch         = [False, '%swelch = %s%s: Whether to use welch windowing and '
                                     'segmenting (True) or not (False)\n']
        self.fast          = [False, '%sfast = %s%s: Whether to use fast FFT computation (True) '
                                     'or not (False). If ``fast = True`` ' 
                                     'only a number of elements equal to a '   
                                     'power of two is taken in the data.\n']
        self.return_phase  = [False, '%sreturn_phase = %s%s: Whether to return the phase values of '
                                     'the spectrum (True) or not (False)\n']
        self.remove_drift  = [False, '%sremove_drift = %s%s: Whether to use remove a linear drift '
                                     'from the data (True) or not (False)\n']
        self.spectrum_type = ['ASD', '%sspectrum_type = %s%s: The type of spectrum requested by the '
                                     'user. It can be ``ASD``, ``PSD``, ``AS``, ``PS``\n']
        ...
        * Possibility to choose whether to calculate the absolute
        ...
        if self.welch[0]:
            ...
            if self.return_phase[0]:
                raise ValueError(f"keyword RETURN_PHASE = True is incompatible with WELCH = True. RETURN_PHASE is set to FALSE")
            ...
            if self.fast[0]:
                nel  = long (2.**(np.floor (np.log(nel) / np.log (2.))))
...
#                norm = np.sqrt (2. * nel / sampfreq)                # PS/PSD normalization
            ...
            dummy = norm * np.abs (ft)/nel # division by nel to be consistent with IDL definition
...
######################################################################
    ...
    def nps(self, array2, sampfreq):
        ...
        if self.fast[0]:
            nlow = 2.**( np.floor( np.log(nlow)/np.log(2.) ) )
        ...
        count        = 1
        j            = np.arange (nlow)
        welch_window = 1 - ((j - (nlow/2.))/(nlow/2.))**2 # welch windowing fnc
    ...
    def spectrum(self, toi, sampfreq):
        ...
        *OUTPUT*
        ...
        if self.welch[0] and self.return_phase[0]:
            raise ValueError(f"keyword /RETURN_PHASE is incompatible with /WELCH. RETURN_PHASE is set to zero")
