...
def lookfor_timevariation(vtime, step_ref=1.5, silent=True):
    ...
    for jj, vmin in enumerate(ids_down):  # hat type
        ...
        if len(rest[rest > 0]) != 0:
            ...
            if cond:
                ...
                amplitude_hat.append(
                    vtime[[idvmax_aux + 1, vmin]].value
                    - vtime[[idvmax_aux, vmin + 1]].value
                )
    if len(idhat) > 0:
    ...
    report = {
        "jumps": len(ids_up),
        "jumps_index": ids_up,
        "jumps_time": vtime[ids_up],
        "downs": len(ids_down),
        "downs_index": ids_down,
        "downs_time": vtime[ids_down],
        "delta_median": TimeDelta(vmedian, scale="tai", format="jd"),
        "delta_ref": TimeDelta(dd_ref, scale="tai", format="jd"),
        "hat": len(idhat),
        "hat_index": idhat,
        "hat_time": time_hat,
        "hat_index_start": idhat_start,
        "hat_index_end": idhat_end,
        "hat_amplitude": amplitude_hat,
        "hat_duration": duration_hat,
        "forward": len(idfor),
        "forward_index": idfor,
        "forward_time": time_for,
        "forward_amplitude": amplitude_for,
        "backward": len(idback),
        "backward_index": idback,
        "backward_time": time_back,
    ...
    *) time comparison with respect to the first hat/forward/backward
    ...
    for jtype in jump_type:
        if report[jtype] > 1:
            ...
            xtime = vtime[xind]
            #
            rep_out[jtype + "_separation_index"] = np.array(xind[1:]) - np.array(
                xind[0:-1]
            )
            ...
            if idfigout is not None:
                ...
                plt.ylabel("Time (sec)")
                plt.plot(
                    rep_out[jtype + "_separationrel_time"].sec,
                    "p--",
                    mec="k",
                    label="Separation (respect to the 1st)",
                )
                plt.plot(
                    rep_out[jtype + "_separation_time"].sec,
                    "p--",
                    mec="k",
                    label="Separation",
                )
...
class timevariation:
    ...
    def plot_timestatus(
        ...
        for kk in kcases:
            ...
            empty_patch1 = mpatches.Patch(
                color="none", label="Number of time jumps:" + str(report["jumps"])
            )
            empty_patch2 = mpatches.Patch(
                color="none", label="Number of time downs:" + str(report["downs"])
            )
            #
            plt.legend(handles=[empty_patch1, empty_patch2], frameon=False)
            plt.plot(xx, tt, ".-", color="C1", alpha=0.7)
            plt.savefig(
                "Fig_" + idcase + "_polarimeter" + kk.split("_")[1] + "_time.png",
                dpi=400,
                bbox_inches="tight",
            )
            plt.close()
            ...
            for jtype in ["hat", "forward", "backward"]:
                ...
                if njtype > 0:
                    ...
                    for ii, idref in enumerate(report[jtype + "_index"]):
                        ...
                        #
                        plt.plot(
                            xx[idref0[0] : idref0[1]],
                            tt[idref0[0] : idref0[1]],
                            "--",
                            alpha=0.8,
                            color="C1",
                            lw=1,
                        ...
                        plt.ylabel("Time [mjd]")
                        empty_patch0 = mpatches.Patch(
                            color="none", label="Time Before: " + tbefore
                        )
                        empty_patch1 = mpatches.Patch(
                            color="none", label="Time After : " + tafter
                        )
    ...
    def grouping(self, idname="NumberCount_group", latex=True, csv=True, silent=False):
        ...
        newc = np.array(
            [
                [cc[0], "{} {} {} {} {}".format(cc[1], cc[2], cc[3], cc[4], cc[5])]
                for cc in count
            ]
        )
